    ...
    const fetchProducts = async () => {
      ...
      switch (sortOption) {
        ...
        case 'price_high':
          ...
          break;
        case 'rating':
          // Extract numeric value before sorting (e.g. "9.3/10" → 9.3)
      ...
      const query = `*[_type == "product" && category == "wheels"] ${orderQuery} {
        _id,
        title,
        price,
        conditionRating,
    ...
    <section className="bg-black text-white w-full min-h-screen">
      <div className="max-w-9xl mx-auto px-4 md:px-16 pt-20 md:pt-28 md:py-12">
        <div className="text-sm md:text-lg text-gray-400 mb-4">
          Home / Collections / <span className="text-white">Wheels</span>
        </div>

        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <h1 className="text-2xl font-semibold">Wheels</h1>
    ...
    <option value="rating">Rating</option>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
          {products.map((product) => (
            ...
            <article
              key={product._id}
              className="bg-black p-2 rounded-sm hover:shadow-md transition duration-300"
            >
